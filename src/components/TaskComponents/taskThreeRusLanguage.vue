<template>
    <div ref="copybook" class="copybook border border-2 border-black rounded-5 overflow-hidden" style="opacity: 1">
      <h1 class="" style="font-family: 'Montserrat', sans-serif; font-size: 3.4rem">
        {{ title }} <span class="fs-4 position-absolute" style="top: 5rem; right: 5rem; font-family: 'Montserrat', sans-serif;">Проверено {{this.currentStudent}} из {{student.length}} тетрадей</span>
      </h1>
      <h3 class="position-absolute fs-1 fw-bold" style="left: .5rem; top: 12rem; rotate: 270deg; font-family: Czizh, sans-serif; color: #1B1464" v-text="derangedStudent[currentStudent]?.name"/>
      <div class="dictation d-flex flex-column mt-4" style="gap: 2rem">
        <div ref="dictations" :key="index" class="ms-5 d-flex align-items-center" v-for="(item, index) in derangedStudent[currentStudent]?.dictation">
          <h1 v-for="word in item" class="fw-bold" style="color: #1B1464; font-family: Czizh, sans-serif" v-text="word"/>
        </div>
      </div>
...
<script>
  export default {
    ...
    data(){
      return{
        title: 'Необходимо проверить диктант',
        timer: '',
        dictionary: [
          'Плясать', 'Голубка', 'Дрожит', 'Далёкий', 'Смельчак', 'Цепной', 'Широкая', 'Скалистый',
        ],
        currentStudent: 0,

        student: [
          {id: 6, name: 'Власов В.', dictation: ['Плесать', 'Голубка', 'Дрожит', 'Далёкий', 'Смельчак', 'Цыпной', 'Широкая', 'Скалистый'], evaluation: ''},
          {id: 14, name: 'Иванов О.', dictation: ['Плясать', 'Голубка', 'Дрожит', 'Далёкий', 'Смельчак', 'Цепной', 'Широкая', 'Скалистый'], evaluation: ''},
          {id: 17, name: 'Калинина А.', dictation: ['Плясать', 'Голубка', 'Дрожит', 'Доликий', 'Смельчак', 'Цепной', 'Широкая', 'Сколистый'], evaluation: ''},
          {id: 25, name: 'Павлова И.', dictation: ['Плесоть', 'Галубка', 'Дрожыт', 'Далёкий', 'Смельчак', 'Цепной', 'Шырокая', 'Сколистый'], evaluation: ''},
          {id: 27, name: 'Савин А.', dictation: ['Плясать', 'Галупка', 'Дражит', 'Далёкий', 'Смельчак', 'Цепной', 'Шерокая', 'Скалистый'], evaluation: ''},
          {id: 29, name: 'Щеглов Д.', dictation: ['Плесать', 'Галупка', 'Дрожит', 'Долёкий', 'Смельчак', 'Цепной', 'Широкая', 'Скалистый'], evaluation: ''},
        ],
    ...
    methods:{
      start(){
        ...
        this.timer = setTimeout(()=>{this.checkDictation();}, 100);
      },
      checkDictation(){
...
<style scoped>
    .copybook{
      background-image: url("../../../public/images/A_notebook in_a_ruler.jpg");
